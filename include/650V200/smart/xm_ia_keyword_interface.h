...
typedef enum
{
    XM_IA_KW_CAR = 0,           /* 车载版本 580不用 */
    XM_IA_KW_HY = 1,            /* 汇云版本 */
    XM_IA_KW_SM = 2,            /* 带屏一体机版本 */
    XM_IA_KW_DR = 3,            /* 行车记录仪版本 */
    XM_IA_KW_DR_SMALL = 4,      /* 行车记录仪small版本 */
    XM_IA_KW_CARE = 8,          /* 老人看护 */
    XM_IA_KW_DOOR = 12,         /* 门锁版本 */
    XM_IA_KW_CAMERA = 16,       /* 相机版本 */

    XM_IA_KW_NUM                /* 关键词检测算法模型个数 */
}XM_IA_KW_TYPE_E;
...
typedef struct
{
    unsigned int uiId;          /* 发音id，关键词模式无效 */
    float flConfidence;         /* 发音得分，关键词模式无效 */
    int tableNum;               /* 发音个数，关键词模式无效 */
    char keyword[25];             /* 关键词结果 */
...
typedef struct
{
    ...
    unsigned char acReserved[16];/* 预留位 */
...
typedef struct
{
    int iFirstFrm;              /* 首帧非首帧标志：1首帧/0非首帧 */
...
typedef struct
{
    ...
    XM_IA_KW_TYPE_E iDetectType;                /* 关键词检测类型(不同类型对应不同分类器) */
    int iTime;                                  /* 窗口时长/ms */
    ...
    unsigned char acReserved[12];               /* 预留位 */
